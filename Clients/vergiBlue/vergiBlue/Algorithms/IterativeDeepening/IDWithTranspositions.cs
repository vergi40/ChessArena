    ...
    internal class IDWithTranspositions : IAlgorithm
    {
            ...
            IBoard board, bool isMaximizing, int timeLimitInMs, CancellationToken stopSearchToken)
        {
            ...
            for (int i = initialSearchDepth; i <= searchDepth; i++)
            {
                ...
                searchResults = MoveOrdering.SortWeightedMovesWithTTSort(searchResults, isMaximizing).ToList();

                if (timeUp) break;
                var pvString = Common.GetPrincipalVariationAsString(searchDepth, board, searchResults.First().move, isMaximizing);
